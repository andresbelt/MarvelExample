...
open class RemoteDataSource {
    open suspend fun <T> safeApiCall(dispatcher: CoroutineDispatcher, apiCall: suspend () -> T): ResponseResult<T> {
    ...
    private fun error(cause : HttpResult, code : Int?, errorMessage : String?) : ResponseResult.Error {
        return ResponseResult.Error(cause, code,  errorMessage)
    }

    private fun parseHttpError(throwable: HttpException) : ResponseResult<Nothing> {
        return try {
